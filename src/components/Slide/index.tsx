...
export default function ({slide}: any) {
      ...
      <Col sm={12} xs={24}>
        <div className={styles.slideLeft}>
          ...
          <Carousel dots={false} autoplay style={{width: '100%'}}>
            {(slide || [1]).map(({title, subTitle, remark, actions, images}: any, index: any) => (<div key={`${index}`}>
              <div className={styles.carouselRow}>
                <h2 className={styles.title}>{title}</h2>
                <span className={styles.subTitle}>{subTitle}</span>
                <div className={styles.remark}>{remark}</div>
                <div key={`${index}`} className={styles.actions}>
                  {(actions || []).map(({icon, title, href}: any, index: any) => (
                    <Button className={styles.btn} type='primary' href={href}>{icon}{title}</Button>))}
                </div>
